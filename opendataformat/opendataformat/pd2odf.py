...
def write_odf(x, path):
    ...
    # Create folder for output .zip in Temp directory
    ...
    if os.path.exists(temp_dir + "/" + filename.removesuffix(".zip")):
        shutil.rmtree(temp_dir + "/" + filename.removesuffix(".zip"))
    
    os.mkdir(temp_dir + "/" + filename.removesuffix(".zip"))

    # write raw data as csv to the output folder
    x.to_csv(temp_dir + "/" + filename.removesuffix(".zip") + "/data.csv", index = False)
    ...
    tree.write(temp_dir + "/" + filename.removesuffix(".zip") + "/metadata.xml", encoding='utf-8', xml_declaration=True)
    
    print("XML file 'metadata.xml' has been created.")
    print("test3")
    dataset_dir = os.path.join(temp_dir, filename.removesuffix(".zip"))
